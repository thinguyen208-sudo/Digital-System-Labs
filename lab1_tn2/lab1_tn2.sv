...
// --- MODULE CHÍNH CHO EXPERIMENT 2 -
module lab1_tn2 (
    input  logic [9:0] SW,      // SW7-0: A, SW9: add_sub [cite: 175]
    input  logic [1:0] KEY,     // KEY0: rst_n, KEY1: clock 
    ...
    logic add_sub_reg; // TÍN HIỆU MỚI ĐÃ THÊM [cite: 114]

    // 1. Thanh ghi lưu giá trị A [cite: 110]
    ...
    // 2. THANH GHI CHO ADD_SUB (Đồng bộ hóa điều khiển)
    dff_en add_sub_sync (
        .d(SW[9]), 
        .clk(KEY[1]), 
        .rst_n(KEY[0]), 
    ...
    // 3. Bộ Cộng/Trừ sử dụng add_sub_reg [cite: 104, 106, 120]
    adder_subtractor_8bit adder_sub_inst (
        .a(S_accumulator), 
        .b(A_reg), 
        .add_sub(add_sub_reg), // THAY ĐỔI TẠI ĐÂY
        .sum(next_res), 
        .cout(c8_sig), 
        .c7_out(c7_sig)
    );

    // 4. Phát hiện lỗi tràn (V = C7 ^ C8) [cite: 45, 146]
    assign v_sig = c7_sig ^ c8_sig;

    // 5. Thanh ghi tích lũy S [cite: 116]
    ...
    // 6. Chốt Carry và Overflow vào LEDR8 và LEDR9 [cite: 181]
    ...
    // 7. Hiển thị lên 7-segment [cite: 182]
